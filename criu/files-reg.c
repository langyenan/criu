...
static int create_link_remap(char *path, int len, int lfd, u32 *idp, struct ns_id *nsid, const struct fd_parms *parms, bool *fb)
{
...
again:
	...
	if (ret < 0 && errno == ENOENT) {
		...
		if (trim_last_parent(link_name) < 0) {
			pr_err("trim failed: @%s@\n", link_name);
			if (fb && parms->fs_type == OVERLAYFS_SUPER_MAGIC) {
				/* In overlayFS, linkat() fails with ENOENT if the removed file is originated from lower layer.
				 * The cause of failure is that linkat() sees the file has st_nlink=0,
				 * which is different than st_nlink=1 we got from earlier fstat() on lfd.
				 * By setting *fb=true, we will fall back to dump_ghost_remap() as it is what should have been
				 * done to removed files with st_nlink=0.
				 */
...
static int dump_linked_remap(char *path, int len, const struct fd_parms *parms, int lfd, u32 id, struct ns_id *nsid, bool *fb)
{
	u32 lid;
	RemapFilePathEntry rpe = REMAP_FILE_PATH_ENTRY__INIT;

	if (create_link_remap(path, len, lfd, &lid, nsid, parms, fb))
		return -1;
...
static int check_path_remap(struct fd_link *link, const struct fd_parms *parms, int lfd, u32 id, struct ns_id *nsid)
{
	...
	if (ret < 0) {
		...
		if (errno == ENOENT) {
			link_strip_deleted(link);
			ret = dump_linked_remap(rpath + 1, plen - 1, parms, lfd, id, nsid, &fb);
			if (ret < 0 && fb) {
				/* fb is true only if following conditions are true:
